...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  if (session.user.role === "admin") {
    ...
    const key = await db.collection("api_keys").findOne({ _id: new ObjectId(params.id) })
    if (!key) {
    ...
    await import("@/lib/security/audit").then(m => m.recordAuditLog({
      ...
      severity: "warning",
      metadata: { keyId: params.id, keyName: key.name },
    }))
  ...
  const success = await revokeApiKey(session.user.id, params.id)
  if (!success) {
    return NextResponse.json({ message: "Clé introuvable." }, { status: 404 })
  }
